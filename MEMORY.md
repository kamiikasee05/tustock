...
- **Monitor Cloud:** `monitor.tustocksoft.com.ar` (Railway). Push-based. Login JWT. 3 tabs (Dashboard/Inventario/Pedidos). POS Remoto. Inventario sin límite de productos (fix 19/8).
- **Admin:** Independiente en `E:\TUSTOCK_ADMIN\`. Habla SOLO a cloud API (Railway). Tray icon púrpura.
- **Última acción:** **Actualización Librería APLICADA y CONFIRMADA (6/8)** — el humano llevó el pendrive F y aplicó el bundle 6/8 (fix rendimiento) con el procedimiento §15. La clienta confirmó: **"el sistema se comportó bien y mejoró la velocidad"**. Cuello de botella restante = **hardware de la clienta** (Intel Celeron J4025 2 núcleos + 4 GB RAM) — el backend responde <100ms a 648 artículos (test de estrés 6/8, `docs/test-estres-2026-08-06.md`) y el frontend React tiene solo 2 backdrop-filters; el límite es Windows+navegador en esa CPU/GPU, no TUSTOCK. **Forense completado (6/8)** sobre `F:\TUSTOCK.zip` (sistema viejo de la librería, `docs/forense-libreria-2026-08-06.md`): la clienta **cargó 639 productos con barcode reales (30.224 unidades) pero registró solo 1 venta** (prueba 29/7) y hay **452 movimientos "exit" sin ventas** — el humano aclaró (6/8): **son ventas reales anotadas al cuaderno** porque Patricia (dueña) decidió que **todo el stock esté cargado antes de empezar a vender por el sistema**. La carga sigue en curso → la capacitación POS queda agendada para cuando termine la carga. Otros hallazgos: bundle viejo con 3 JS huérfanos (bug §15 vigente); `.env` del bundle incluye `TUSTOCK_ADMIN_TOKEN` (riesgo bajo — router admin NO montado en main.py — sanitizar en próximo build); no usa categorías (1 sola "LIBRERIA").
- **Última acción 2 (6/8):** **MODO LIVIANO implementado + bundle v2 listo** — el humano pidió optimizar el sistema para la PC débil de la clienta. DEV implementó el modo liviano (detección de hardware débil `navigator.hardwareConcurrency <= 2` OR `deviceMemory <= 4` → clase `lite` en `<html>` que desactiva las 69 animaciones/transiciones inline + el glass `backdrop-filter`, y también se activa con `prefers-reduced-motion: reduce`). Build `index-BY0B1i1D.js`/`index-st6KUokJ.css`. Dispatcher: EXE rebuild (11.06 MB) — **fix BOM en `.env`** (Set-Content UTF8 con BOM rompía el parseo de dotenv y hacía fallar el build de PyInstaller: error `SystemExit: 1` en config.py:31; reescrito sin BOM) + **`.env` empaquetado sanitizado** (sin `TUSTOCK_ADMIN_TOKEN` — directiva del forense, backup en `server/.env.bak` restaurado en dev). **Bundles actualizados en maestra `E:\USB_TUSTOCK\TUSTOCK` y pendrive `F:\TUSTOCK`** (procedimiento §15 con rename, verificado: python312.dll en `_internal`, sin anidado, hashes nuevos, `cloud.json` de la librería preservado en F:, 4 APKs, docs; **los `_internal_viejo` residuales de ambos USBs se pudieron borrar**). Instructivo `F:\ACTUALIZAR-LIBRERIA.txt` actualizado (header bundle v2 + sección "Optimizar la PC"). **Checklist de optimización Windows** creado: `docs/optimizacion-pc-libreria-2026-08-06.md` + `F:\OPTIMIZAR-PC-LIBRERIA.txt` (apagar efectos de animación — que además activa el modo liviano vía prefers-reduced-motion —, plan alto rendimiento, inicio limpio, cerrar pestañas, limpiar disco, updates fuera de horario). **Pendiente: el humano lleva el pendrive F a la librería para aplicar el bundle v2 + checklist (no urgente — la clienta ya notó la mejora del fix 6/8).**
- **Última acción 3 (19/8):** **FIX CLOUD PUSH — retención + límite 500 eliminado**. La librería tiene **914 productos** (vs 891 reportados el 14/8: +23 productos nuevos). El Monitor Cloud solo mostraba 500 por límite hardcoded. DEV quitó el límite y agregó limpieza automática de pushes viejos (7 días, 1/día). Commit `d3429f2` en master. **Deploy pendiente** — Railway con incidente de deploys demorados (US East, desde 18/8). Deployar ~20/8. Humano tiene **suscripción Opencode GO** (a documentar).
- **Directiva estratégica (6/8):** **"Tratar al sistema como si fuera a correr en una PC con MENOS de los requisitos mínimos"** — regla permanente de diseño del humano. Toda feature, todo build y todo bundle se optimiza/empaqueta ANTICIPÁNDOSE a hardware débil (Celeron 2 núcleos + 4 GB RAM como la librería), no asumiendo una PC moderna. **Limpieza aplicada el mismo día:** bundle v3 sin `cloudflared.exe` (51.7 MB — feature de tunnel en desuso, el launcher lo descarga on-demand) → **bundle base 106 → 54.8 MB**; APKs debug eliminados de los USBs (solo release, 2 APKs) → **pendrive F: 215 → 108 MB**; DBs de test del arranque borradas de F. Spec actualizado (script `scripts_datas` filtra cloudflared.exe, `installer/tustock.spec`). Build verificado (health OK), bundles v3 en maestra + F (cloud.json preservado).
...
19. **Directiva estratégica — "PC por debajo de los requisitos mínimos" (6/8):** El humano definió como regla permanente de diseño que **todo se optimiza y empaqueta como si el sistema fuera a correr en una PC con MENOS de los requisitos mínimos** (referencia: Intel Celeron J4025 2 núcleos + 4 GB RAM de la clienta de la librería). Aplica a: (a) **frontend** — animaciones/transiciones/glass solo si no degradan la fluidez; el modo liviano (`index.css` `.lite` + `prefers-reduced-motion`) es el default para hardware débil y NO debe romperse con features nuevas; (b) **backend** — endpoints paginados, sin N+1, sin cargas completas innecesarias; (c) **bundle/empaquetado** — nada de binarios grandes innecesarios (lección: `cloudflared.exe` 51.7 MB era el 48% del bundle base y se descargaba on-demand); antes de incluir un archivo en `installer/tustock.spec`, justificar el peso; (d) **APKs** — solo release en los USBs de entrega, debug solo para desarrollo. Cualquier feature nueva debe pasar la prueba mental: "¿esto se siente fluido en la PC de la clienta?"
...
| 2026-08-19 | **Cloud push fix aprobado**: Retención de pushes (7 días, 1/día) + eliminación del límite de 500 productos. La librería tiene 914 productos y solo se veían 500. Fix beneficia a todos los clientes actuales y futuros. Deploy pendiente (Railway incidente). | 🧑 HUMANO + DEV |
| 2026-08-19 | **Suscripción Opencode GO**: El humano tiene una suscripción a OpenCode GO (plan de pago del IDE). Documentar para tracking de costos. | 🧑 HUMANO |
...
- [ops] PENDRAVE F: SINCRONIZADO CON MAESTRA (4/8): El pendrive físico `F:\TUSTOCK` estaba **DESACTUALIZADO** (EXE 29/7, JS `index-TLgyBOjS.js`, APK #42 del 30/7) mientras la maestra `E:\TUSTOCK\USB_TUSTOCK\TUSTOCK` tenía el bundle 3/8 (EXE 11.6 MB, JS `index-BxsJ1R9z.js`, APK #48 con modo toma/auditoría). Sincronizado: rename `_internal`→`_internal_viejo` (sin borrar bloqueados), copia de maestra (EXE + `_internal` + 4 APKs #48 3/8 22:36-22:37), `config/cloud.json` preservado de la librería (api_key e0e20f...), restos limpios (`apks/` 21/7 + `_internal_viejo` borrados). **DB regenerada limpia** (la de la maestra traía datos de prueba de la validación E2E del 25/7: Rexona, Tester, Gotita — borrada; primera ejecución creó base: trial 30d hasta 3/9, EULA pendiente, 0 productos). Verificado end-to-end: EXE arranca desde el pendrive, health OK, `/api/products?page=1&page_size=50` responde `{total:0,...}`, index.html apunta a `index-BxsJ1R9z.js`, python312.dll en raíz, SIN `_internal\_internal` anidado, cloud.json restaurado. **Pendrive LISTO para actualizar la librería.** (2026-08-04)
- [ops] INSTRUCTIVO ACTUALIZACIÓN LIBRERÍA ACTUALIZADO (4/8): `F:\ACTUALIZAR-LIBRERIA.txt` reescrito al bundle 3/8 — header actualizado (bundle 3/8 + APK release #48 modo Toma/Auditoría), Paso 4 nota DB limpia del bundle, Paso 9 verificación de features nuevas (página "Importar stock", Auditorías con Sobrantes/Faltantes), Paso 11 novedades app Stock (switch Auditoría, nombre+precio al escanear código nuevo, import CSV), problema común nuevo (catálogo sin stock en app), backup renombrado a `BACKUP-TUSTOCK-2026-08-04`. 11.4 KB. (2026-08-04)
- [telenota] APK STOCK NO GENERA CSV (4/8 20:43): El humano reportó por audio a TELENOTAS que "la APK de stock no está levantando los datos correctamente, no se pudo generar el SBC/CSV". Verificación Dispatcher: la lógica de generación de CSV **SÍ existe en el repo** (`android/app/src/stock/java/com/tustock/scanner/CsvToma.kt` — `createCsvFile()`, `appendLine()`, `readCsv()` + share `ACTION_SEND`/`EXTRA_STREAM` en StockMainActivity.kt). El reporte apunta a la APK compilada (quizás build viejo o bug runtime). **DECISIÓN DEL HUMANO: IGNORAR esta telenota por ahora** — no se procesa como tarea, queda registrada para evaluación futura. No delegada a DEV. (2026-08-04)
- [verificado] APK STOCK CSV — CAUSA = ERROR HUMANO (4/8 23:xx): El humano reinstaló la APK #48 del USB y confirmó que **el CSV sí se genera** — el problema era que no recordaba que al tocar "Finalizar y enviar" se abre el menú de compartir (buscaba un botón que envíe el CSV directo). Verificación completa Dispatcher de la APK del USB (hash MD5 `E7ED35BB...` idéntico a maestra, manifest binario UTF-16 con FileProvider `com.tustock.stock.fileprovider` + StockMainActivity, classes2.dex con CsvToma/createCsvFile/header `barcode;cantidad`/prefijos `toma-stock-`/`auditoria-`, res/8K.xml = file_paths compilado con `<files-path name="tomas" path="tomas/">`). Revisión funcional línea por línea de `StockMainActivity.kt`: flujo Iniciar toma → escanear → cantidad (+ nombre/precio si nuevo) → Guardar (escribe CSV) → Finalizar y enviar (share) OK; sesión se retoma tras crash (`restoreSession`/`persistSession`). **Riesgo detectado:** `doShareCsv()` usa `type="text/csv"` — en celulares sin apps para ese MIME, `ActivityNotFoundException` → se siente como "no generó CSV". Plan: prueba en campo en librería (5/8). (Dispatcher + Humano, 2026-08-04)
- [android] FIX SHARE MIME + IME ENTER EN APP STOCK (4/8): Pedido del humano tras el error del CSV. **Fix 1 — `doShareCsv()` en `StockMainActivity.kt`:** primer intento con `type="text/csv"`, ante `ActivityNotFoundException` reintenta con `type="*/*"`, si el segundo falla Toast "No hay apps para compartir archivos instaladas". try/catch en cada startActivity. **Fix 2 — navegación con Enter:** `android:imeOptions="actionNext"` en `newProductNameInput` (XML:239) y `newPriceInput` (XML:253), `actionDone` en `quantityInput` (XML:270) con `setOnEditorActionListener` que ejecuta `addStockBtn.performClick()` (mismo flujo que "Guardar") + oculta teclado (InputMethodManager). IDs verificados: `newProductNameInput`, `newPriceInput`, `quantityInput`. Flujo: nombre → precio → cantidad → Enter guarda. NO tocó src/pos/ ni server/web. Commit `016d658` pusheado → **CI run 30963836621 ✅ PASÓ (3m1s), APKs #49** (4 APKs stock+pos debug+release). (DEV, 2026-08-04)
- [ops] APKS #49 EN USBS + INSTRUCTIVO ACTUALIZADO (4/8 23:xx): APKs #49 (fix share MIME + Enter) descargados de release `apk-49` y copiados a maestra `E:\TUSTOCK\USB_TUSTOCK\TUSTOCK` y pendrive `F:\TUSTOCK` (hash stock-release `8D792CEC...` verificado idéntico en ambos). El primer copy con glob `*.apk` no aplicó (hashes seguían #48 `E7ED35BB...`) — recopiado archivo por archivo OK. `F:\ACTUALIZAR-LIBRERIA.txt` actualizado: APK #49, novedades Enter entre campos + share robusto en Paso 11. Pendiente: prueba en campo librería (5/8) con el flujo completo Iniciar toma → escanear → cantidad → Guardar → Finalizar y enviar → WhatsApp. (Dispatcher, 2026-08-04)
- [fix] RENDIMIENTO POS + PRODUCTOS + PRESUPUESTOS (6/8): La clienta de la Librería reportó sistema lento con **648 artículos** al buscar producto en POS y Gestión de Productos. Causa raíz (verificado): el frontend cargaba TODOS los productos al abrir — `loadAllProducts()` con loop `page_size=200` (4+ requests secuenciales) + `GET /stock` completo (648 items) en POS y Presupuestos, y Products cargaba `GET /stock` completo al montar. El backend ya estaba optimizado (paginación + search ILIKE + stock batched desde 3/8) — NO se tocó. **Fix en 4 archivos:** `Sales.tsx` (POS) — eliminado `loadAllProducts` y `GET /stock` del mount (solo sales+customers); debounce 300ms → `GET /products?search=X&page_size=15` con dropdown de sugerencias (nombre, código, precio, stock, click agrega al carrito); Enter → `GET /products/scan/{code}` (toast "Código no encontrado"); sin `GET /stock` post-venta. `Presupuestos.tsx` — mismo patrón (suggestions + scan endpoint). `Products.tsx` — eliminado `GET /stock` del mount, `getStock()` reemplazado por `p.stock` (campo que ya trae el backend). `client.ts` — `Product.stock` + interface `ScanResult`. Build ✅ `index-B74xO_2t.js` (340.48 kB). QA Dispatcher: diff revisado (limpieza total, sin referencias muertas), EXE rebuild 11.06 MB + health OK, maestra y pendrive F actualizados (python312.dll en raíz, sin `_internal\_internal`, cloud.json preservado en F:), instructivo `F:\ACTUALIZAR-LIBRERIA.txt` actualizado al bundle 6/8 con pasos de verificación del fix. **Pendiente: el humano lleva el pendrive F a la librería y aplica el procedimiento de actualización (§15).** (DEV + Dispatcher, 2026-08-06)
- [test] TEST DE ESTRÉS BACKEND (6/8): La clienta de la Librería reportó lentitud con 648 artículos (ver fix 6/8 arriba). Para anticipar catálogos grandes, DEV creó `server/seed_stress.py` (seed masivo executemany en DB separada `tustock_stress.db` vía `TUSTOCK_DB`, producción intacta) + `server/benchmark_stress.py` (9 endpoints, 27 medidas, p50/p95/p99, serial + 5 threads). **DEV se colgó a mitad** (scripts con bug: `_` en vez de `p2`/`p3` al desempaquetar → NameError; server colgado sin matar) — **el Dispatcher lo completó**: fix del bug, corrió los 3 volúmenes (1k/5k/20k; el de 100k se descartó a pedido del humano), limpió DBs. **Resultados clave:** (1) ✅ **fix 6/8 validado** — `products_list` paginado estable: ~30ms serial / ~75ms concurrente incluso a 20k; `products_scan` 6-25ms en todos los volúmenes; a 648 artículos todo responde <100ms. (2) 🔴 **`GET /api/stock` es el cuello de botella**: serial 47ms (1k) → 908ms (20k), concurrente 465ms → **10.5s (20k)**; ya no lo usa nadie en el frontend (fix 6/8 lo eliminó) → recomendación: paginar o eliminar del backend (~2h). (3) 🟡 **search LIKE %x%** degrada lineal: 8ms (1k) → 71ms (20k) serial, 468ms concurrente a 20k → FTS5 si hay clientes con miles (~4-6h). (4) 🟡 sales_create lock de escritura SQLite: p95 613-688ms concurrente — OK para uso humano real. 0 errores en todos los endpoints. Reporte completo en `docs/test-estres-2026-08-06.md`. Recomendaciones NO implementadas (solo cuando haya clientes con miles de artículos). (Dispatcher, 2026-08-06)
- [exito] ACTUALIZACIÓN LIBRERÍA APLICADA + CONFIRMADA POR LA CLIENTA (6/8): El humano llevó el pendrive F a la librería y aplicó el bundle 6/8 (fix rendimiento POS/Productos/Presupuestos) con el procedimiento §15. La clienta confirmó: **"el sistema se comportó bien y mejoró la velocidad"**. Cuello de botella restante = **hardware**: la PC es un **Intel Celeron J4025 (2 núcleos @2.0GHz) + 4 GB RAM (3,75 usable)** — Windows x64. El backend responde <100ms a 648 artículos (test de estrés 6/8) y el frontend React tiene solo 2 `backdrop-filter` en `index.css` → **el límite es la CPU/GPU + navegador, no TUSTOCK**. Acción recomendada: checklist de optimización Windows (animaciones off, inicio limpio, Edge modo eficiencia) — NO es desarrollo. (Humano + Dispatcher, 2026-08-06)
- [forense] FORENSE INSTALACIÓN VIEJA LIBRERÍA — `F:\TUSTOCK.zip` (6/8): El humano copió el sistema anterior (bundle 3/8, EXE 11.6MB, JS `index-BxsJ1R9z.js`) ANTES de la actualización. Análisis read-only de la DB real en `docs/forense-libreria-2026-08-06.md`. **Hallazgos:** (1) 🔴 **la clienta cargó 639 productos con barcode EAN reales (30.224 unidades, promedio 47.3, todos con precio — avg $5.691 — y min_stock) pero registró SOLO 1 venta** (prueba 29/7, $3.500 fiado, cliente "TEST"); hay **452 movimientos "exit" sin ventas** — **ACLARACIÓN DEL HUMANO (6/8): son ventas reales anotadas al cuaderno**, decisión de Patricia (dueña): todo el stock cargado antes de empezar a vender por el sistema → la carga sigue en curso → **capacitación POS agendada para cuando termine la carga** (no es "no adopción"). (2) 1 pedido pendiente RECHAZADO (reglas Wero, 29/7 — probó la app POS y lo rechazó a los 20s: verificar si el flujo le confundió). (3) 2 vendedores cargados (LUMA DNI 33234567, PATRI TOLEDO DNI 25976207) → pensaba usar la app POS de vendedores. (4) 0 auditorías, 0 vencimientos (librería), **1 sola categoría "LIBRERIA"** → no explota informes por categoría. (5) Bundle viejo con **3 JS huérfanos** en dist (kBkTtofU + TLgyBOjS + BxsJ1R9z) → bug §15 punto 4 vigente. (6) `.env` del bundle shippeado incluye `TUSTOCK_ADMIN_TOKEN` → **riesgo BAJO** (router admin NO montado en main.py — verificado: no hay `include_router(admin_router)`), pero sanitizar en próximo build (maestra E: y pendrive F: tampoco lo tienen sanitizado). Pendientes accionables: capacitación POS a la clienta (🔥), sanitizar .env + limpiar JS huérfanos en próximo build, sugerir categorías. (Dispatcher, 2026-08-06)
- [feature] MODO LIVIANO FRONTEND (6/8): Optimización para la PC débil de la clienta (Celeron J4025 2 núcleos + 4 GB RAM). **Detección de hardware débil** en `web/src/main.tsx` (`navigator.hardwareConcurrency <= 2` OR `navigator.deviceMemory <= 4` → `document.documentElement.classList.add('lite')`) + **CSS en `web/src/index.css`**: la clase `.lite` y `@media (prefers-reduced-motion: reduce)` desactivan las **69 animaciones/transiciones inline** de los componentes (Layout, Dashboard, Products, Sales, Toast, etc.) con `animation-duration: 0.01ms !important` y eliminan el `backdrop-filter: blur(12px)` del `.glass-panel` (caro en GPU integrada). Cero impacto en lógica, cero cambios de layout. Build ✅ `index-BY0B1i1D.js` (340.89 kB) + `index-st6KUokJ.css` (4.0 kB). QA Dispatcher verificado (rg en index.css/main.tsx + dist). (DEV, 2026-08-06)
- [ops] EXE REBUILD + FIX BOM EN .env (6/8): El build de PyInstaller falló 2 veces con `SystemExit: 1` en `server/config.py:31` (TUSTOCK_TOKEN "no configurado") **porque `Set-Content -Encoding UTF8` de PowerShell 5.1 escribió BOM (EF BB BF)** al inicio del `.env` y dotenv no parseaba la primera línea. Fix: reescrito con `[System.IO.File]::WriteAllLines(..., UTF8Encoding($false))` — sin BOM, import OK (token len 19). **`.env` empaquetado SANITIZADO** (sin `TUSTOCK_ADMIN_TOKEN` — directiva del forense; backup en `server/.env.bak` restaurado en dev con admin len 40). EXE nuevo 11.06 MB, arranque verificado (health OK), DB de test limpiada del bundle. **Los `_internal_viejo` residuales de maestra y pendrive F se pudieron borrar** (DLLs ya no bloqueados) — quedaron ambos USBs sin basura. (Dispatcher, 2026-08-06)
- [ops] BUNDLES V2 EN MAESTRA + PENDRIVE F (6/8): Bundle con modo liviano copiado a `E:\USB_TUSTOCK\TUSTOCK` y `F:\TUSTOCK` vía procedimiento §15 con rename (lección 3/8). Verificado en ambos: EXE 11.06 MB, `_internal\python312.dll` ✅, sin `_internal\_internal` anidado, index.html apunta a `index-BY0B1i1D.js`/`index-st6KUokJ.css`, **cloud.json de la librería preservado en F:** (api_key e0e20f...), 4 APKs intactos, docs intactos (configurar.bat, Guia PDF, KEY.txt, LEEME.txt). Cero basura residual. (Dispatcher, 2026-08-06)
- [docs] CHECKLIST OPTIMIZACIÓN PC LIBRERÍA (6/8): `docs/optimizacion-pc-libreria-2026-08-06.md` + copia texto plano `F:\OPTIMIZAR-PC-LIBRERIA.txt`. 6 pasos de Windows sin instalar nada: (1) 🔥 apagar "Efectos de animación" (Accesibilidad) — además activa el modo liviano de TUSTOCK vía prefers-reduced-motion, (2) plan de energía Alto rendimiento, (3) inicio limpio (Task Manager → Aplicaciones de inicio), (4) cerrar programas/pestañas al usar TUSTOCK (4 GB RAM), (5) Liberador de espacio, (6) Windows Update fuera de horario. Qué NO hacer + verificación post-ajuste (CPU en reposo 0-5%, navegación sin fades). **Instructivo `F:\ACTUALIZAR-LIBRERIA.txt` actualizado**: header bundle v2 + modo liviano explicado + sección "Optimizar la PC" con resumen de los 6 pasos. (Dispatcher, 2026-08-06)
- [decision] DIRECTIVA ESTRATÉGICA "PC POR DEBAJO DE LOS REQUISITOS MÍNIMOS" (6/8): El humano definió como regla permanente de diseño: **"tratar al sistema como si fuera a correr en una PC con MENOS de los requisitos mínimos"** — anticiparse a hardware débil (Celeron 2 núcleos + 4 GB RAM), no asumir PC moderna. Quedó como regla 19 en §7 (aplica a frontend, backend, bundle y APKs). Toda feature nueva pasa la prueba: "¿se siente fluido en la PC de la clienta?" (Humano, 2026-08-06)
- [ops] LIMPIEZA BUNDLE v3 (6/8): Auditado el peso del bundle v2 (215 MB): **`cloudflared.exe` (51.7 MB) era el 48% del bundle base** — feature de tunnel en desuso (la clienta usa Monitor Cloud push, no tunnel local) y el launcher lo descarga on-demand (`launcher.py::download_cloudflared`). DEV excluyó el binario del spec (`installer/tustock.spec` → variable `scripts_datas` que filtra `cloudflared.exe`, sintaxis verificada con ast.parse, 17/18 archivos de scripts conservados). **APKs debug (55 MB) eliminados de USBs** (solo release se instala; debug queda solo para desarrollo). **DBs de test del arranque borradas del pendrive F** (tustock.db 4096 + wal 218KB + shm — basura de mi test de salud). Resultado: **bundle base 106 → 54.8 MB; pendrive F: 215 → 108 MB** (EXE 11.06 + _internal 43.7 + 2 APKs release 52.9 + docs). Build v3 verificado (health OK, sin cloudflared en bundle, .env sanitizado sin BOM, sin DB de test, python312.dll OK, sin anidado). Bundles v3 copiados a maestra `E:\USB_TUSTOCK\TUSTOCK` (107.7 MB) y `F:\TUSTOCK` (107.7 MB, cloud.json preservado). (DEV + Dispatcher, 2026-08-06)
- [ops] LIMPIEZA REPO + COMMIT SESIÓN (7/8): Auditoría completa del working dir (1.092 MB → **582 MB**, -510 MB de basura). Borrado: `OpenCode Desktop Installer.exe` (118 MB, instalador accidental en raíz), `scripts/cloudflared.exe` (51.7 MB, launcher lo descarga on-demand), `installer/dist` (106 MB, build viejo con cloudflared) + `installer/build` (36.7 MB), `build/` (36.7 MB), `TUSTOCK_DEV/` (104 MB, bundle dev residual), APKs debug de `android/` (55 MB). **PDFs de presentación del sistema** (29/7, material de ventas) movidos de la raíz a `docs/marketing/`. **TUSTOCK_DEMO actualizado a bundle v3** (§15): sin cloudflared, sin `_internal_viejo` (2 residuos borrados), solo APKs release, DB demo preservada (118.784 bytes, 48 productos, trial hasta 23/8) — verificado end-to-end: health OK + `/api/products` total 48 (Aceite/Agua/Arroz). `.gitignore` ampliado: `*.env.bak` + `server/.env.bak` (el backup con `TUSTOCK_ADMIN_TOKEN` NUNCA va al repo). Repo trackeado verificado limpio (solo `server/.env.example`). Commit: modo liviano + spec + docs 6/8 (forense, test-estrés, optimización) + scripts de estrés + MEMORY + .gitignore. (Dispatcher, 2026-08-07)
...
- [fix] IMPORT REGISTRA CANTIDAD INMEDIATA (3/8): El humano reportó que tras "Registrar todos los pendientes" los productos se creaban con nombre y precio pero el stock quedaba en 0. Diagnóstico: el flujo era de DOS pasos — (1) "Registrar todos" creaba el producto + AuditItem con `counted_qty` pero NO tocaba el stock; (2) "Aplicar correcciones" (botón verde abajo) recién ahí escribía `CurrentStock`. El humano confirmó: **el botón "Registrar todos" debe registrar TODO de una (nombre + precio + cantidad)**, sin depender del paso extra. Fix en `server/services/csv_import.py`: `register_product_from_import` y `register_products_batch` ahora crean `CurrentStock(product_id, quantity)` inmediatamente al registrar (si qty > 0, tras `db.refresh`/`db.flush` — necesario por `autoflush=False` en database.py:18). `theoretical` ahora refleja la cantidad aplicada → `difference = 0` → `complete_audit` no genera movimientos espurios (0 movimientos en verificación). Web (`ImportStock.tsx`): confirm de "Registrar todos" actualizado — dice que aplica nombre + precio + stock contado. Verificado end-to-end: batch register → `current_stock` = 7 ANTES de completar; complete → `discrepancies: []`, 0 movimientos. "Aplicar correcciones" sigue válido solo para productos existentes (matched) y ediciones de conteo. Build web ✅ `index-D68GbARn.js` (334.86 kB). EXE rebuild (11.06 MB) + USB principal y demo actualizados + verificación estructural OK (python312.dll en raíz, sin `_internal\_internal`, DB base/demo preservadas, 4 APKs). (DEV + Dispatcher, 2026-08-03)
- [idea] TOGGLE AUDITORÍA EN APP STOCK (3/8): Nueva idea del humano tras el fix: **la app Stock debería conservar el toggle para auditoría** (que existía antes de la reescritura Opción E), donde se cargue el catálogo offline y se realice el mismo flujo de toma de stock (CSV local). Hoy la app tiene un solo flujo unificado (toma CSV: escanear → cantidad → nombre/precio si es nuevo). La idea: switch "Auditoría" para diferenciar el propósito (contar productos existentes vs registrar nuevos en la toma). Pendiente: clarificar comportamiento exacto con el humano y evaluar con DEV. NO desarrollado aún — queda como idea en espera. (Humano + Dispatcher, 2026-08-03)
- [validado] IMPORT CSV VALIDADO POR EL HUMANO EN CAMPO (3/8): El humano probó la app Stock + import y confirmó: **"funciona genial, los datos se cargan correctamente"**. Fix de cantidad inmediata validado en uso real. Toggle de auditoría queda pendiente — el humano se va a dormir y seguimos en unas horas. (Humano + Dispatcher, 2026-08-03)
- [decision] TOGGLE AUDITORÍA — DIRECCIÓN CONFIRMADA + ETAPA 2 AGENDADA (3/8): El humano volvió y detalló la idea: (1) al activar el toggle, la app descarga el catálogo; (2) durante el flujo permite ingresar la cantidad; (3) hace la comparativa para la auditoría. DEV estimó **Etapa 1 ~6.5h** (toggle + comparativa en vivo "Sistema: X | Contado: Y → Diferencia: Z" con acumulador por producto + resumen al finalizar; el CSV NO cambia — toggle 100% UI) y **Etapa 2 ~15h** (endpoint monthly-comparison + tabla comparativa mes a mes + vista mensual + alerta "rojo" 2+ auditorías consecutivas + index en audit_items.audit_id). **Decisiones del humano: (1) Etapa 1 ARRANCA YA. (2) Etapa 2 se agenda para DENTRO DE UN MES (ver cómo integrar el reporte). (3) Alcance Etapa 2 ampliado: diferencias REAL vs TEÓRICO (necesarias) + datos importantes como PRODUCTOS MÁS VENDIDOS y PRODUCTOS SIN MOVIMIENTO hace meses.** Riesgos DEV Etapa 2: "pérdida" debe llamarse "variación entre conteos" (hay ventas/compras entre conteos que el reporte no ve — no cerraría con el cuaderno del cliente); "no auditado" ≠ "contado 0" (celda vacía, no cero); precio actual como proxy de valor monetario. El reporte no tiene datos hasta 2+ tomas en meses distintos. (Humano + Dispatcher + DEV, 2026-08-03)
- [feature] TOGGLE AUDITORÍA APP STOCK — ETAPA 1 IMPLEMENTADA (3/8): `android/app/src/stock/` — switch "Auditoría" en top bar (thumb #4d8eff) + comparativa en vivo en la tarjeta de resultado: "Sistema: X | Contado: Y → ±Z" con color por signo (rojo faltante #EF4444 / verde sobrante #22C55E / gris neutro #94A3B8). Acumulador `csvAccum: HashMap<String, Double>` (barcode → cantidad) reemplaza `csvItems: HashSet` — se reconstruye del CSV tras crash (`CsvToma.readCsv` ahora agrega cantidades por barcode con `unquote()`). Toggle ON: descarga catálogo si no está (toast online/cache/none), `audit_mode` persistido en prefs. Resumen al finalizar: "Finalizar y enviar" muestra dialog Resumen de auditoría (contados, con diferencia, faltantes/sobrantes + hasta 5 ejemplos) antes del share; "Nueva toma" lo inserta en el dialog existente. CSV SIN CAMBIOS (header `barcode;cantidad;nombre;precio;fecha`). Sin imports nuevos, sin tocar POS/server/web. Commit `4c348bf` + push → **CI APK #46 compilado con success** (artifacts stock+pos, debug+release descargados a ambos USBs 16:23). (DEV, 2026-08-03)
- [qa] FLUJO AUDITORÍA VALIDADO END-TO-END (3/8): Server dev con DB demo (52 prod). CSV de prueba con formato exacto de la app (`barcode;cantidad;nombre;precio;fecha`, 5 productos existentes + 1 nuevo). **Flujo completo OK:** (1) `import-csv` → audit draft con diferencias calculadas (theoretical vs counted, ej. Aceite teorico 24 vs contado 9 → dif -15); (2) `import-register-batch` → producto nuevo registrado con stock + precio (Galletitas Surtidas x6, code TST3166349743, qty 5, $1500); (3) `complete` → discrepancias guardadas + correcciones aplicadas al stock. **Segunda corrida con los mismos datos dio diferencias 0 → confirma que las correcciones se aplican** (stock quedó al valor contado). El ciclo completo app → CSV → import → diferencias → Auditorías queda validado: la app genera el CSV, la web ya muestra la columna "Diferencia" en el preview (`ImportStock.tsx:382`) y la página Auditorías las lista con colores (`Audits.tsx:174-180`). No hace falta nada nuevo para "registrar" diferencias — se guardan en `audit_items` (theoretical/counted/difference) con su fecha, que es lo que alimentará el reporte mensual (Etapa 2). (Dispatcher, 2026-08-03)
- [bug] CATÁLOGO SIN STOCK EN APP STOCK (3/8): El humano probó el APK #46 con modo Auditoría y reportó 2 problemas: (a) el catálogo se cargaba pero **sin cantidades teóricas** (comparativa "Sistema: 0" siempre); (b) **no se podía volver a la toma de stock normal** (el toggle parecía no poder desactivarse). **Bug (a) — causa raíz:** `GET /api/products` (el que usa la app para el catálogo, `ApiClient.downloadCatalog()`) NO incluía el campo `stock` en el JSON: `to_product_out()` no lo agregaba y el schema `ProductOut` no lo tenía → `ProductResponse.stock: Double` (ApiClient.kt:19) se deserializaba como 0.0. El web lo resuelve aparte con `GET /stock` (Products.tsx:51), por eso nadie lo había notado. **Bug (b) — causa raíz UX:** `thumbTint="#4d8eff"` se aplicaba SIEMPRE (ON y OFF) → el switch parecía siempre encendido; apagarlo no daba feedback; y la comparativa solo corre con `csvActive=true` (si el toggle se activaba sin toma iniciada no pasaba nada visible). (Humano, 2026-08-03)
- [fix] CATÁLOGO CON STOCK + UX TOGGLE AUDITORÍA (3/8): **Backend (`server/routes/products.py` + `schemas.py`):** `ProductOut` ahora tiene `stock: float = 0.0`; `to_product_out(p, stock=None)` acepta el stock; `list_products` hace **un solo query batched** `CurrentStock.product_id.in_(ids)` → `stock_map` (cero N+1, incluso a page_size=200); `get_product`/`create_product`/`update_product` pasan `get_current_stock(db, p.id)`. Verificado: `/api/products?page=1&page_size=3` devuelve stock (Aceite=9.0, Agua=20.0, Arroz=6.0) — valores coinciden con la DB dev. Impacto web: NINGUNO (el web ignora campos extra, `GET /stock` intacto). **App Stock (`StockMainActivity.kt` + `activity_stock_main.xml`):** `updateAuditUi()` — thumb azul #4d8eff ON / gris #94a3b8 OFF, track #334155 ON / #1e293b OFF, label "Auditoría" azul activo / gris inactivo (quitados thumbTint/trackTint estáticos del XML); `updateAuditStatus()` — nuevo TextView pill `@id/auditStatus` bajo la top bar que muestra "Modo auditoría — iniciá la toma para ver la comparativa" cuando auditMode ON sin sesión; toasts al togglear: OFF "Modo normal — toma de stock" / ON "Modo auditoría — comparativa activa". Sin loop de listener en onCreate (isChecked se setea ANTES del listener). CSV intacto, POS intacto. (DEV, 2026-08-03)
- [feature] RESUMEN SOBRANTES/FALTANTES EN AUDITORÍAS (3/8): Pedido del humano: "En auditorías veo el registro de las auditorías completadas, pero mínimamente deberíamos mostrar sobrantes y faltantes." **Backend (`server/services/audit_service.py::list_audits`):** un solo query agregado (outerjoin StockAudit→AuditItem, GROUP BY, `func.count(AuditItem.id)`, `func.count(AuditItem.counted_qty)`, `func.sum(case((diff>0,1)))`, `func.sum(case((diff<0,1)))`) — cero N+1. Cada auditoría ahora incluye `total_items`, `items_checked`, `sobrantes` (`difference > 0`), `faltantes` (`difference < 0`). Sintaxis posicional `case((cond,1), else_=0)` (SQLAlchemy 2.x rechaza la lista legacy `case([...])` con ArgumentError). Verificado: auditoría #5 → faltantes 5, #1 → sobrantes 1, #6 → 0/0. **Frontend (`web/src/api/client.ts` + `web/src/pages/Audits.tsx`):** interface `Audit` ampliada; 2 columnas nuevas (Sobrantes/Faltantes) después de Estado con badges verdes (`rgba(80,216,144,0.12)`/`var(--success)`) y rojos (`rgba(244,81,108,0.12)`/`var(--error)`), sobrantes con signo `+`, `—` si 0 o no completada; empty state colSpan 6→8. **Detalle read-only de completadas:** `viewAudit()` (solo `GET /audits/{id}`, sin tocar estado) + botón "Ver detalle" para `completed`; en el detalle se ocultan "Contar +1" y "Completar y aplicar", chips de sobrantes/faltantes en header. Build web ✅ `index-BxsJ1R9z.js` (337.06 kB). Commit `1748664`. (DEV + Dispatcher, 2026-08-03)
- [feature] DOS MODOS EN APP STOCK — TOMA vs AUDITORÍA (3/8): Pedido del humano: la app Stock tiene que distinguir bien TOMA DE STOCK (carga inicial / agregar stock) de AUDITORÍA (genera CSV que al importar genera diferencias). **CsvToma.kt:** `createCsvFile(context, auditMode)` — nombre de archivo `auditoria-YYYYMMDD-HHMM.csv` en modo auditoría, `toma-stock-YYYYMMDD-HHMM.csv` en modo toma (2.4); BOM + header `barcode;cantidad;nombre;precio;fecha` + quoting RFC 4180 intactos (la web importa igual). **StockMainActivity.kt (70 líneas):** `modeBanner` TextView SIEMPRE visible bajo el switch — verde "TOMA DE STOCK — para carga inicial o agregar stock" / azul "AUDITORÍA — Teórico: N artículos por contar"; `theoreticalCount()` = productos del catálogo con `stock > 0` (2.1); botón dinámico "Iniciar toma"/"Iniciar auditoría"; **guard de toggle mid-toma**: bloquea cambiar de modo con una toma en curso (revierte el switch + toast "Finalizá o enviá la toma actual antes de cambiar de modo") — evita CSV abierto en un modo etiquetado en otro; `auditSummaryText()`: artículos contados, teórico, con diferencia, faltantes/sobrantes en SKUs Y unidades + hasta 5 ejemplos + aviso fuerte si `scanned < theoretical` (2.2): "⚠️ IMPORTANTE: escaneaste X de N artículos con stock. Hay productos que NO contaste — revisá antes de enviar."; dialog "Vista previa de auditoría" (2.3); share con subject/chooser según modo; session bar con prefijo TOMA DE STOCK·/AUDITORÍA· (3). XML: área superior en contenedor vertical (top bar + modeBanner + auditStatus + sessionBar), sin marginTop absolutos. Edge: sin catálogo (sin red y sin cache) el banner avisa "sin catálogo con stock (descargá el catálogo)". Commit `d77c273` → **CI APK #48 SUCCESS (4/8)**: 4 APKs (stock+pos debug+release) descargados y copiados a ambos USBs (principal + demo). **El humano validó la APK y le encantó: "FUNCIONAL y EXPLICITA"** — quedó aprobada. **Pendiente: prueba en campo en la librería (mañana 4/8, el humano lleva el USB)**. (DEV + Dispatcher, 2026-08-03)
- [fix] CLOUD PUSH — RETENCIÓN + LÍMITE 500 ELIMINADO (19/8): La Librería GREEN tiene 914 productos pero el Monitor Cloud solo mostraba 500 (límite hardcoded en `LIMIT 500` de `collect_inventory()`). Además, NO existía limpieza de pushes viejos → fuga de almacenamiento ~624 MB/día (~18 GB/mes). **DEV implementó:** (A) `_cleanup_old_pushes()` en `cloud/api.py:675-710` — después de cada push, borra los >7 días y conserva 1 diario de los últimos 7 días → cap de ~1.6 MB por cliente; (B) `LIMIT 500` eliminado de `cloud/agent.py:88` y `server/cloud_push.py:148`; (C) POS tab `per_page` actualizado de 500 → 2000 en `cloud/dashboard.html:416`. Commit `d3429f2` pusheado a `origin/master`. **Deploy PENDIENTE** — Railway con incidente de deploys demorados/fallidos (US East, desde 18/8 23:19 UTC). Deployar cuando se resuelva (~20/8). (DEV + Dispatcher, 2026-08-19)
