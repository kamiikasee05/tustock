...
- **Última acción:** Fix rendimiento POS/Productos/Presupuestos (6/8) — clienta Librería reportó lentitud con 648 artículos. Eliminada carga completa de productos + `GET /stock` del frontend: búsqueda server-side con sugerencias (debounce 300ms) + scan endpoint. Build `index-B74xO_2t.js`, EXE 11.06 MB, bundles maestra + pendrive F actualizados, instructivo `F:\ACTUALIZAR-LIBRERIA.txt` al bundle 6/8. Pendiente: llevar el pendrive F a la librería y aplicar la actualización (§15).
